...
sys.path.append('/workspace/GossipFL')
...
def add_args(parser):
    ...
    parser.add_argument("--worker_number", type=int, default=None)
...
if __name__ == "__main__":
    ...
    with raise_MPI_error():
        ...
        if args.rank is None:
            logging.info("args.rank is None, use process_id as rank")
            cfg.rank = process_id
            cfg.worker_number = worker_number
        else:
            logging.info("args.rank is not None, use args.rank as rank")
            cfg.rank = args.rank
            process_id = cfg.rank
            worker_number = args.worker_number
        logging.info("cfg.args ================ %s" % (str(cfg)))
        if cfg.algorithm in ['FedAvg', 'AFedAvg', 'PSGD', 'APSGD', 'Local_PSGD', 
                             'FedSGD']:
            cfg.client_index = process_id - 1
        elif cfg.algorithm in ['DPSGD', 'DCD_PSGD', 'CHOCO_SGD', 'SAPS_FL', 'FLOCK']:
        ...
        if cfg.algorithm == 'FedAvg':
            ...
            from algorithms.DPSGD.DPSGD_API_randomk import FedML_init, FedML_DPSGD
            ...
            from algorithms.CHOCO_SGD.CHOCO_SGD_API_randomk import FedML_init, FedML_CHOCO_SGD
            FedML_CHOCO_SGD(process_id, worker_number, device, comm,
                                model, train_data_num, train_data_global, test_data_global,
                                train_data_local_num_dict, train_data_local_dict, test_data_local_dict, cfg,
                                model_trainer)
        elif cfg.algorithm == 'SAPS_FL':
            from algorithms.SAPS_FL.SAPS_FL_API_randomk import FedML_init, FedML_SAPS_FL
            FedML_SAPS_FL(process_id, worker_number, device, comm,
                                model, train_data_num, train_data_global, test_data_global,
                                train_data_local_num_dict, train_data_local_dict, test_data_local_dict, cfg,
                                model_trainer)
        elif cfg.algorithm == 'FLOCK':
            from algorithms.FLOCK.FLOCK_API import FedML_init, FedML_FLOCK
            FedML_FLOCK(process_id, worker_number, device, comm,
                                model, train_data_num, train_data_global, test_data_global,
                                train_data_local_num_dict, train_data_local_dict, test_data_local_dict, cfg,
                                model_trainer)
        else:
