...
def _safe_local_device(mapped_global_idx=None):
    if not torch.cuda.is_available():
        return torch.device("cpu")
    n = torch.cuda.device_count()  # NOTE: comment translated from Chinese
    # NOTE: comment translated from Chinese
    use = 0 if n <= 1 else (0 if mapped_global_idx is None else min(mapped_global_idx, n - 1))
    torch.cuda.set_device(use)
    return torch.device(f"cuda:{use}")
            ...
            for host, gpus_util_map_host in gpu_util.items():
                for gpu_j, num_process_on_gpu in enumerate(gpus_util_map_host):
                    for _ in range(num_process_on_gpu):
                        ...
                        i += 1
            logging.info("1Process %d running on host: %s,gethostname: %s, gpu: %d ..." % (
...
def init_training_device_from_gpu_util_parse(process_id, worker_number, gpu_util_parse):
    logging.info("Mapping processes to GPU devices from gpu_mapping.py ..init_training_device_from_gpu_util_parse.")
        ...
        for host, gpus_util_map_host in gpu_util.items():
            for gpu_j, num_process_on_gpu in enumerate(gpus_util_map_host):
                for _ in range(num_process_on_gpu):
                    gpu_util_map[i] = (host, gpu_j)
                    i += 1
        logging.info("2Process %d running on host: %s,gethostname: %s, gpu: %d ..." % (
            process_id, gpu_util_map[process_id][0], socket.gethostname(), gpu_util_map[process_id][1]))
        # assert i == worker_number
        i == worker_number

        device = torch.device("cuda:" + str(min(gpu_util_map[process_id][1], torch.cuda.device_count() - 1)) if torch.cuda.is_available() else "cpu")
