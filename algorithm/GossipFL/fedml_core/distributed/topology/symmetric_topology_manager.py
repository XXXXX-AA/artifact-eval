...
class SymmetricTopologyManager(BaseTopologyManager):
    ...
    def generate_topology(self):
        ...
        topology_ring = np.array(nx.to_numpy_array(nx.watts_strogatz_graph(self.n, 2, 0)), dtype=np.float32)
        # print(topology_ring)

        # randomly add some links for each node (symmetric)
        k = int(self.neighbor_num)
        # print("undirected_neighbor_num = " + str(k))
        topology_random_link = np.array(nx.to_numpy_array(nx.watts_strogatz_graph(self.n, k, 0)), dtype=np.float32)
