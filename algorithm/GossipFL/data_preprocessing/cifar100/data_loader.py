...
def get_dataloader(dataset, datadir, train_bs, test_bs, dataidxs=None, args=None, download=False):
    return get_dataloader_CIFAR100(datadir, train_bs, test_bs, dataidxs, args=args, download=download)
...
def get_dataloader_CIFAR100(datadir, train_bs, test_bs, dataidxs=None, args=None, download=False):
    ...
    train_dl = data.DataLoader(dataset=train_ds, batch_size=train_bs, shuffle=True,num_workers=0, pin_memory=False, drop_last=True)
    test_dl = data.DataLoader(dataset=test_ds, batch_size=test_bs, shuffle=False, num_workers=0, pin_memory=False, drop_last=True)
...
def load_partition_data_cifar100(dataset, data_dir, partition_method, partition_alpha, client_number, batch_size,
    ...
    rank = _get_rank(args)
    logging.info("load_partition_data_cifar10: rank = %d" % rank)
    # NOTE: comment translated from Chinese
    from pathlib import Path

    current_dir = Path(__file__).parent
    part_path = current_dir / "partitions" / str(client_number) / "cifar100_part.json"
