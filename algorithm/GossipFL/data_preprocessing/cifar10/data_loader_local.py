...
def load_partition_data_cifar10(dataset, data_dir, partition_method, partition_alpha, client_number, batch_size,
    ...
    #        data_local_num_dict, train_data_local_dict, test_data_local_dict, class_num
        dataidxs = np.array(net_dataidx_map[client_index])
        np.random.shuffle(dataidxs)
        split = int(len(dataidxs) * 0.7)
        train_idxs = dataidxs[:split]
        test_idxs = dataidxs[split:]
        local_data_num = len(dataidxs)
        local_test_num = len(test_idxs)
        logging.info("client_index = %d, local_sample_number = %d, local_test_num = %d" % (
            client_index, local_data_num, local_test_num))

        data_local_num_dict[client_index] = len(train_idxs)
        train_data_local, _ = get_dataloader(dataset, data_dir, batch_size, batch_size, train_idxs, args=args)
        _, test_data_local = get_dataloader(dataset, data_dir, batch_size, batch_size, test_idxs, args=args)
