import logging

from mpi4py import MPI


from utils.timer_with_cuda import Timer
from utils.metrics import Metrics
from utils.logger import Logger
from fedml_core.distributed.topology.symmetric_topology_manager import SymmetricTopologyManager

from .decentralized_worker import DecentralizedWorker

from .decentralized_worker_manager_auto_backpressure import DecentralizedWorkerManager # use this one for FLOCK paper -4-
# from .decentralized_worker_manager_auto_backpressure_randomk import DecentralizedWorkerManager    # use this one for FLOCK with random-k communication
# from .decentralized_worker_manager import DecentralizedWorkerManager
# from .decentralized_worker_manager_init import DecentralizedWorkerManager # ablation study: push-sum with utility-refresh and choose-best -2-
# from .decentralized_worker_manager_ablation_bw_only import DecentralizedWorkerManager # ablation study: only bandwidth consideration, autobp -3-
# from .decentralized_worker_manager_ablation_pure_push_sum import DecentralizedWorkerManager # ablation study: pure push-sum without backpressure -1-
from .MyModelTrainer import MyModelTrainer

from .FLOCK_topology_manager import FLOCKTopologyManager

track_time = True


def FedML_init():
    comm = MPI.COMM_WORLD
    process_id = comm.Get_rank()
    worker_number = comm.Get_size()
    return comm, process_id, worker_number


def FedML_FLOCK(process_id, worker_number, device, comm, model, train_data_num, train_data_global, test_data_global,
                 train_data_local_num_dict, train_data_local_dict, test_data_local_dict, args, model_trainer=None):
    # args = Args(job)
    # initialize the topology (ring)
    if model_trainer is None:
        model_trainer = MyModelTrainer(model, device, args)
    model_trainer.set_id(process_id)

    # configure logger.
    # conf.logger = logging.Logger(conf.checkpoint_dir)

    # configure timer.
    timer = Timer(
        verbosity_level=1 if track_time else 0,
        log_fn=Logger.log_timer
    )
    metrics = Metrics([1], task=args.task)

    # tpmgr = SymmetricTopologyManager(worker_number, 2)
    # tpmgr.generate_topology()

    # generate topology in the loop
    # tpmgr = SAPSTopologyManager(args=args)
    # tpmgr.generate_topology(t=0)
    tpmgr = None
    # logging.info(tpmgr.topology)

    # initialize the decentralized trainer (worker)
    worker_index = process_id
    # self.topology_manager.generate_topology(t=self.global_round_idx)
    worker = DecentralizedWorker(worker_index, tpmgr, train_data_global, test_data_global, train_data_num,
                 train_data_local_dict, test_data_local_dict, train_data_local_num_dict, worker_number, 
                 device, model, args, model_trainer, timer, metrics)

    client_manager = DecentralizedWorkerManager(args, comm, process_id, worker_number, worker, tpmgr, model_trainer,
                                                timer, metrics)
    client_manager.run()
