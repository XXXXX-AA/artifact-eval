...
class DecentralizedWorkerManager(BaseDecentralizedWorkerManager):
    def __init__(self, args, comm, rank, size, worker, topology_manager, model_trainer, timer, metrics):
        ...
        self._stale_counts = [0] * (len(self._stale_bins) + 1)
        self._stale_seen = 0
        self._stale_log_interval_sec = float(getattr(args, "staleness_log_interval_sec", 10.0))
        ...
        # self._bp_enabled = bool(getattr(args, "auto_backpressure", False))
        ...
        self._bp_max_ack_delay_sec = float(getattr(args, "auto_bp_max_ack_delay_sec", 0))
    ...
    def handle_msg_from_neighbor(self, msg_params):
        """Enqueue; actual disk write handled by _rx_loop to avoid blocking training."""
        sender_id = msg_params.get(MyMessage.MSG_ARG_KEY_SENDER)
        sender_round = msg_params.get(MyMessage.MSG_ARG_KEY_LOCAL_ROUND)
        if sender_round is not None:
            self._record_staleness(sender_round)
        ack_ready = False
        try:
    ...
    def _record_staleness(self, sender_round):
        try:
            st = int(self.round) - int(sender_round)
            if st < 0:
                st = 0
        except Exception:
            return
        self._stale_seen += 1
        idx = 0
        for edge in self._stale_bins:
            if st <= edge:
                break
            idx += 1
        self._stale_counts[idx] += 1
        now = time.time()
        if (self._stale_sample_every > 0 and (self._stale_seen % self._stale_sample_every == 0)) or (
            self._stale_log_interval_sec > 0 and (now - self._stale_last_log_t) >= self._stale_log_interval_sec
        ):
            self._log_staleness(now)

    def _log_staleness(self, now=None):
        total = sum(self._stale_counts)
        if total <= 0:
            return
        parts = []
        for i, edge in enumerate(self._stale_bins):
            parts.append(f"<= {edge}:{self._stale_counts[i]}")
        parts.append(f"> {self._stale_bins[-1]}:{self._stale_counts[-1]}")
        logging.info("[rank %s] staleness dist (n=%s): %s", self.worker_index, total, ", ".join(parts))
        self._stale_counts = [0] * (len(self._stale_bins) + 1)
        self._stale_seen = 0
        if now is None:
            now = time.time()
        self._stale_last_log_t = now
        ...
        while not self._rx_stop.is_set():
            ...
            try:
                ...
                def _decode_job(item, _sender_id, _sender_round):
                    """
                    Decompress on CPU in a thread pool and build a lightweight decoded dict:
                    Carry only sender/round and decompressed q_values/q_indices.
                    Note: clear items (release tensor refs) here to avoid races with the main thread.
                    """
                    self.msg_received += 1
    ...
    def _agg_loop(self):
        """
        Pop from decompressed queue -> call add_result_for_flock (no-decompression fast path) ->
        Release tokens (allow next decompression). Keep decompressed resident <= 3 and avoid push-sum loss.
        """
        cnt = 0
