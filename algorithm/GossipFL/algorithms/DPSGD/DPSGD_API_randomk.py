...
class OverlayMaskedRingTopologyManager(SymmetricTopologyManager):
    def __init__(self, n, overlay_mask=None, rng=None, max_tries=2000, time_limit_s=10.0, restarts=200):
        super().__init__(n, neighbor_num=2)
        self.overlay_mask = overlay_mask
        self._rng = rng if rng is not None else np.random.default_rng()
    ...
    # def _find_ring_order(self, allowed: np.ndarray):
    #     n = allowed.shape[0]
    #     if n <= 2:
    ...
    def _build_adj_list(self, allowed: np.ndarray):
        n = allowed.shape[0]
    ...
    def _remaining_feasible(self, adj, visited, start, current):
        n = len(adj)
        for i in range(n):
            if visited[i]:
                continue
            cnt = 0
            for v in adj[i]:
                if not visited[v] or v == start or v == current:
                    cnt += 1
                    if cnt >= 2:
                        break
            if cnt < 2:
        ...
        while stack:
            u = stack.pop()
            for v in adj[u]:
                if not visited[v] or v == start:
                    if not seen[v]:
                        seen[v] = True
                        stack.append(v)
        for i in range(n):
            if not visited[i] and not seen[i]:
                return False
        return True

    def _enhanced_ring_order(self, adj):
        n = len(adj)
        if n == 0:
            return []
        if n == 1:
        ...
        def dfs(start, current, visited, path):
            if time.perf_counter() - start_time > self._time_limit_s:
                return False
            if len(path) == n:
                return start in adj[current]

            if not self._remaining_feasible(adj, visited, start, current):
                return False

            candidates = [v for v in adj[current] if not visited[v]]
            if not candidates:
                return False
            candidates.sort(key=lambda v: deg[v])

            for v in candidates:
                visited[v] = True
                path.append(v)
                if dfs(start, v, visited, path):
                    return True
                path.pop()
                visited[v] = False
            return False
        ...
        for _ in range(self._restarts):
            ...
            visited[start] = True
            path = [start]
            if dfs(start, start, visited, path):
                return path
        return None
    ...
    def generate_topology(self):
        ...
        if n == 2:
            if allowed[0, 1]:
                ...
                logging.warning("overlay mask blocks 0-1 edge; ring not possible for n=2")
        elif n > 2:
            adj_list = self._build_adj_list(allowed)
            # order = self._find_ring_order(allowed)
            order = self._enhanced_ring_order(adj_list)
            if order is None:
                # logging.warning(
                #     "failed to build overlay ring after %d tries; falling back to OverlayMaskedSymmetricTopologyManager",
                #     self._max_tries,
                # )
                # fallback = OverlayMaskedSymmetricTopologyManager(
                #     n, 2, overlay_mask=self.overlay_mask, rng=self._rng
                # )
                # fallback.generate_topology()
                # self.topology = fallback.topology
                # return
                raise RuntimeError(
                    f"failed to build overlay ring within {self._time_limit_s}s "
                    f"({self._restarts} restarts); check overlay mask connectivity"
                )
            for i in range(n):
                u = order[i]
                v = order[(i + 1) % n]
...
def FedML_DPSGD(process_id, worker_number, device, comm, model, train_data_num, train_data_global, test_data_global,
    ...
    tpmgr = OverlayMaskedRingTopologyManager(worker_number, overlay_mask=mask)
