        ...
        with raise_MPI_error():
            for epoch in range(self.epochs):
                ...
                for iteration in range(self.worker.num_iterations):
                    ...
                    param_nbits=get_n_bits(flatten_params.buffer)
                    # if self.worker_index == 0 and iteration  == 0:
                    #     logging.info("Worker {}: iteration {} epoch {} param_nbits {}".format(
                    #     self.worker_index, iteration, epoch, param_nbits))
                    ...
                    for neighbor_idx in self.topology_manager.get_out_neighbor_idx_list(self.worker_index):
                        if self.compression in ["randomk", "topk"]:
                            ...
                            # logging.info("Worker %d,  Iteration %d, n_bits: %.2f" %
                            #          (self.worker_index, self.iteration, n_bits))  
                ...
                # if self.worker_index == 0:
                #     logging.info('Local Training Epoch: {} \t n_bits: {:.6f}'.format(
                #                 epoch, n_bits))
