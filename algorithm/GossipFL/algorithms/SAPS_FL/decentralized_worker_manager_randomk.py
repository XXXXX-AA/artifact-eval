...
from .SAPS_topology_manager_randomk import SAPSTopologyManager
...
class DecentralizedWorkerManager(BaseDecentralizedWorkerManager):
    ...
    def handle_msg_from_neighbor(self, msg_params):
        ...
        if self.worker.check_whether_all_receive():
            ...
            # self.__train()        
        # else:
        #     logging.info("worker %d, not receive all neighbor results" % (self.worker_index))
            ...
            for epoch in range(self.epochs):
                ...
                for iteration in range(self.worker.num_iterations):
                    ...
                    if self.args.Failure_chance is not None and np.random.rand(1) < self.args.Failure_chance:
                        ...
                        # logging.info("Worker Index = {}, Epoch = {}, Iteration = {}, Loss = {}".format(
                        #     self.worker_index, self.epoch, self.iteration, loss))
                    ...
                    # reset the neighbor_hat_params for storing new values
                    self.worker.init_neighbor_hat_params()
                    self._rebuild_receive_flags()  # NOTE: comment translated from Chinese
    ...
    # NOTE: comment translated from Chinese
    def _rebuild_receive_flags(self):
        # NOTE: comment translated from Chinese
        # for neighbor_idx in self.worker.in_neighbor_idx_list:
        #     self.worker.flag_neighbor_result_received_dict[neighbor_idx] = False
        expected = [int(n) for n in self.worker.in_neighbor_idx_list]  # NOTE: comment translated from Chinese
        self.worker.flag_neighbor_result_received_dict = {n: False for n in expected}
        # expected = set(map(int, self.worker.in_neighbor_idx_list))
        # NOTE: comment translated from Chinese
        # self.worker.flag_neighbor_result_received_dict = {nid: False for nid in expected}
        # logging.info(f"Rebuild receive flags for worker {self.worker_index}: expected neighbors = {self.worker.in_neighbor_idx_list}")
        # logging.info(f"Rebuild receive flags: {self.worker.flag_neighbor_result_received_dict}")
