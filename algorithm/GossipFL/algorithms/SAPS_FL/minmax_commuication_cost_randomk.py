...
def minmax_communication_match(bandwidth, roles, no_ask=1, islimited=True):
    # print("minmax_communication_match", roles)
    nworkers = len(bandwidth)
    ...
    match = None
    while(1):
        # print("x: %d, left: %d, right: %d"%(x, left, right))
        ...
        if flow == int(nworkers/2):
            ...
            right = x
        # print("match, bandwidth_threshold", match, bandwidth_threshold)
...
#     def __init__(self, bandwidth, B_thres, T_thres):
#         self.bandwidth = torch.FloatTensor(bandwidth)
#         self.n_workers = self.bandwidth.size(0)
...
# NOTE: comment translated from Chinese
# NOTE: comment translated from Chinese
#         bw_min, bw_max = self.bandwidth.min(), self.bandwidth.max()
#         bw_norm = (self.bandwidth - bw_min) / (bw_max - bw_min + 1e-8)
#         similarity = 1 - torch.abs(bw_norm.unsqueeze(1) - bw_norm)
# NOTE: comment translated from Chinese
...
# NOTE: comment translated from Chinese
#         valid_dims = torch.sum(eigenvalues < 1e-5).item()
#         n_clusters = max(2, min(valid_dims, int(np.sqrt(self.n_workers))))
        
# NOTE: comment translated from Chinese
#         if eigenvectors.size(1) < n_clusters:
#             padding = torch.zeros((self.n_workers, n_clusters - eigenvectors.size(1)))
#             embedding = torch.cat([eigenvectors, padding], dim=1)
#         else:
#             embedding = eigenvectors[:, :n_clusters]
        
#         return self._torch_kmeans(embedding, n_clusters).numpy()

#     def _torch_kmeans(self, X, k, max_iter=100):
# NOTE: comment translated from Chinese
...
# NOTE: comment translated from Chinese
# NOTE: comment translated from Chinese
#             valid_mask = ~torch.isnan(X_flat).any(dim=1)
#             valid_X = X_flat[valid_mask]
#         else:
            
# NOTE: comment translated from Chinese
#             valid_mask = ~torch.isnan(X).any(dim=2).any(dim=1)
#             valid_X = X[valid_mask]
#         if len(valid_X) < k:
#             k = len(valid_X)
        
# NOTE: comment translated from Chinese
#         centroids = valid_X[torch.randperm(len(valid_X))[:k]]
...
#                 new_centroids.append(cluster_points.mean(dim=0) if len(cluster_points) > 0 else centroids[i])
...
# NOTE: comment translated from Chinese
#     np.random.seed(42)
#     bandwidth = np.random.exponential(scale=1.0, size=(10, 10))
#     np.fill_diagonal(bandwidth, 0)
    
# NOTE: comment translated from Chinese
#     saps = SAPS_gossip(bandwidth, B_thres=0.5, T_thres=0.3)
# NOTE: comment translated from Chinese
...
class SAPS_gossip(object):
    # 'bandwidth_type': 'random', 'B_thres': 3.0, 'T_thres': 3
    ...
    def generate_match(self, t):
        ...
        # filter out zero-bandwidth edges (do not select bw=0 links)
        B_unmatch = B_unmatch * (self.B_origin > 0)
        ...
        if (match==None) or len(match) != self.n_workers:
            ...
            # filter out zero-bandwidth edges (do not select bw=0 links)
            B_unmatch2 = B_unmatch2 * (self.B_origin > 0)
...
# class SimpleGossipMatch:
#     def __init__(self, B_origin, B_thres, T_thres):
#         self.n_workers = len(B_origin)
#         self.B_origin = B_origin
#         self.B_thres = B_thres
#         self.T_thres = T_thres
#         self.R_stamps = np.zeros([self.n_workers, self.n_workers])
...
#     def generate_match(self, t):
# NOTE: comment translated from Chinese
#         allowed_links = (self.B_origin >= self.B_thres).astype(int)

# NOTE: comment translated from Chinese
#         nodes_need_connect = self.get_nodes_over_time(t)
        
# NOTE: comment translated from Chinese
#         all_nodes = np.arange(self.n_workers)
#         np.random.shuffle(all_nodes)

# NOTE: comment translated from Chinese
#         match = {}
#         unpaired = set(all_nodes)

# NOTE: comment translated from Chinese
#         while len(nodes_need_connect) >= 2:
#             n1 = nodes_need_connect.pop()
#             n2 = nodes_need_connect.pop()
#             if allowed_links[n1, n2]:
#                 match[n1] = n2
#                 match[n2] = n1
#                 unpaired.discard(n1)
#                 unpaired.discard(n2)
...
# NOTE: comment translated from Chinese
#         unpaired.update(nodes_need_connect)
...
# NOTE: comment translated from Chinese
#         real_bandwidth_threshold = 1e9
#         for i in match:
#             j = match[i]
# NOTE: comment translated from Chinese
#                 bw = self.B_origin[i, j]
#                 if bw < real_bandwidth_threshold:
#                     real_bandwidth_threshold = bw

#         return match, real_bandwidth_threshold

#     def get_nodes_over_time(self, t):
#         """
# NOTE: comment translated from Chinese
#         """
#         nodes = []
#         for i in range(self.n_workers):
# NOTE: comment translated from Chinese
#             last_communication_times = self.R_stamps[i]
# NOTE: comment translated from Chinese
#             if np.all(last_communication_times < t - self.T_thres):
#                 nodes.append(i)
#         return nodes
...
#                     paired.update([n1, n2])
# NOTE: comment translated from Chinese
#                     all_nodes[i + 1], all_nodes[j] = all_nodes[j], all_nodes[i + 1]
#                     found_pair = True
#                     break
...
# NOTE: comment translated from Chinese
#         unpaired = [n for n in all_nodes if n not in paired]

#         for n in unpaired:
# NOTE: comment translated from Chinese
#             if len(paired) > 0:
#                 partner = np.random.choice(list(paired))
#                 match[n] = partner
# NOTE: comment translated from Chinese
#             else:
# NOTE: comment translated from Chinese
...
# NOTE: comment translated from Chinese
# NOTE: comment translated from Chinese
#     print("Original Bandwidth Matrix:\n", B_origin)

#     matcher = FullMatchGossip(B_origin, B_thres=50, T_thres=5)

#     for t in range(10):
#         match, min_bw = matcher.generate_match(t)
#         print(f"Round {t}:")
#         print("Match:", match)
#         print("Min bandwidth this round:", min_bw)
