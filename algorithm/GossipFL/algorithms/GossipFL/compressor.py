...
class SAPS_FLSparsificationCompressor(object):
    ...
    def uncompress(self, msg_params, weight, hat_params, selected_shapes, original_shapes, device):
        ...
        selected_shapes = msg_params.get(MyMessage.MSG_ARG_KEY_SELECTED_SHAPES)  # NOTE: comment translated from Chinese
        # round = msg_params.get(MyMessage.MSG_ARG_KEY_LOCAL_ROUND)
        
        if selected_shapes is None:
            logging.warning("Message missing SELECTED_SHAPES; fallback to default.")
            selected_shapes = [np.prod(s) for s in original_shapes]
        elif torch.is_tensor(selected_shapes):      
            selected_shapes = selected_shapes.tolist()
        ...
        # NOTE: comment translated from Chinese
        KAPPA = 2.0  # NOTE: comment translated from Chinese
        cos_sim = torch.tanh(KAPPA * cos_raw).item()  # NOTE: comment translated from Chinese
        # NOTE: comment translated from Chinese
