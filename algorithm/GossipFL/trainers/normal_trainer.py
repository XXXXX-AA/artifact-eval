...
class NormalTrainer(ModelTrainer):
    ...
    def train_one_step(self, train_batch_data, device, args, epoch=None, iteration=None, tracker=None, metrics=None, **kwargs):
        ...
        if (tracker is not None) and (metrics is not None):
            ...
            metric_stat = metrics.evaluate(loss, output, labels)
            tracker.update_metrics(metric_stat, n_samples=labels.size(0))
            logging.debug('(Trainer_ID {}. Train epoch: {}, iteration: {} '.format(
    ...
    # NOTE: comment translated from Chinese
    # NOTE: comment translated from Chinese
    #         acc_int = int(round(avg_acc * 100))        # e.g. 0.9876 → 99
    # NOTE: comment translated from Chinese
    # NOTE: comment translated from Chinese
    #         if metrics is not None:
    # NOTE: comment translated from Chinese
    #             summary_dict = {"Loss": avg_loss, "Acc1": acc_display}
    #             summary_msg  = metrics.str_fn(summary_dict)
    #         else:
    # NOTE: comment translated from Chinese
    #             summary_msg = f'loss: {avg_loss:.6f}  acc: {acc_display}'

    #         logging.info(
    #             f'(Trainer_ID {self.id}) Test epoch {epoch} Summary {summary_msg}')
